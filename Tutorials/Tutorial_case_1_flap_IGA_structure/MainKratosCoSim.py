...
class CustomCoSimulationAnalysis(CoSimulationAnalysis):
    ...
    def OutputSolutionStep(self):
        ...
        if not hasattr(self, "tracking_conditions_initialized"):
            ...
            self.point_A = (9.5, 6.03)  # Replace with your target coordinates
            self.point_B = (9.5, 5.97)
        ...
        for i, node in enumerate(condition.GetNodes()):
            solution_A += node.GetSolutionStepValue(KratosMultiphysics.DISPLACEMENT_Y) * N[0, i]
        ...
        solution_B = 0.0
        for i, node in enumerate(condition.GetNodes()):
            solution_B += node.GetSolutionStepValue(KratosMultiphysics.DISPLACEMENT_Y) * N[0, i]

        self.disp_x_history_B.append(solution_B)

    def Finalize(self):
        super().Finalize()
        print("Finalizing and plotting results...")
